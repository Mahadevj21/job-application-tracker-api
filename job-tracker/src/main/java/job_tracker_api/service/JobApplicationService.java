...
public class JobApplicationService {
    ...
    public JobApplicationDto createJob(JobApplicationDto dto) {
        ...
        job.setUser(user);
        return toDto(jobApplicationRepository.save(job));
    }
    ...
    public Page<JobApplicationDto> getAllJobs(Pageable pageable,
        ...
        if (status != null && !status.isBlank()) {
            ...
            return jobApplicationRepository.findByStatus(jobStatus, pageable).map(this::toDto);
        }
        if (companyName != null && !companyName.isBlank()) {
            return jobApplicationRepository.findByCompanyNameContainingIgnoreCase(companyName, pageable).map(this::toDto);
        }
        if (jobTitle != null && !jobTitle.isBlank()) {
            return jobApplicationRepository.findByJobTitleContainingIgnoreCase(jobTitle, pageable).map(this::toDto);
        }
